...
export function layersModal(models, ui, config) {
  ...
  var getLayersForProjection = function(projection) {
    var filteredLayers = lodashValues(config.layers)
  ...
  var init = function() {
    ...
    models.compare.events.on('change', drawDefaultPage);
  ...
  var addLayer = function(event) {
    ...
    model.add(decodeURIComponent($(this).val()), {}, layerGroupStr);
  };
  var layerGroupStr = getActiveLayerGroupString(
    models.compare.active,
    models.compare.isCompareA
  ...
  var removeLayer = function(event) {
    event.stopPropagation();
    model.remove(decodeURIComponent($(this).val()), layerGroupStr);
  };

  var onLayerRemoved = function(layer) {
    var $element = $(
      self.selector + ' [data-layer="' + util.jqueryEscape(layer.id) + '"]'
    );
